...
public class RateMovieTest {
    ...
    @Test
    public void ForbiddenWatchListTest() {
        String url = "http://138.197.181.131:5000/api/movies";
        String movieListPageHtml = "";
        try {
            movieListPageHtml = HTTPRequestHandler.getRequest(url);
            int statusCode = HTTPRequestHandler.getStatusCode(url);
            Assert.assertEquals(statusCode, 403);
            Assert.assertEquals(movieListPageHtml, "Unauthorized");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
